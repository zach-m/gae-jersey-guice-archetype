...
public class InfoAPI
{
	...
	private static final String POWERED_BY_URL = "http://www.example.com";
	private static final String POWERED_BY_NAME = "example.com";
	...
	public String getEnvironmentalHtml()
	{
		...
		{
			sb.append("<pre style='color:red'>Error:${symbol_escape}n").append(e.getMessage()).append("${symbol_escape}n").append(timeStamp).append("</pre>");
			e.printStackTrace();
		}
	...
	private static String createStyle()
	{
		...
		sb.append("body{font-family:'Proxima Nova Regular','Segoe UI',Roboto,'Droid Sans','Helvetica Neue',Arial,sans-serif;font-style:normal;font-weight:400;padding:10px;overflow:hidden;line-height:1.46em;color:${symbol_pound}333}");
		...
		sb.append("th{font-weight:400;font-size:13px;color:${symbol_pound}039;background:${symbol_pound}b9c9fe;padding:8px}");
		sb.append("td{background:${symbol_pound}e8edff;border-top:1px solid ${symbol_pound}fff;color:${symbol_pound}669;padding:8px;word-break:break-all;min-width:250px}");
		sb.append("tbody tr:hover td{background:${symbol_pound}d0dafd}");
		return sb.toString();
	}
	...
	private void printGaeProperties(StringBuilder sb)
	{
		...
		if (userLoggedIn)
		{
			String logoutUrl = "Click <a href=${symbol_escape}"" + userService.createLogoutURL(request.getRequestURI()) + "${symbol_escape}">here</a> to Logout";
			sb.append("<tr><td>").append("createLogoutURL()").append("</td><td>").append(logoutUrl).append("</td></tr>");
		}
		else
		{
			String loginUrl = "Click <a href=${symbol_escape}"" + userService.createLoginURL(request.getRequestURI()) + "${symbol_escape}">here</a> to Login";
